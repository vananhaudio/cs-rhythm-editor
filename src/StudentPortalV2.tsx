...
// ─── Mock data ────────────────────────────────────────────────────────────────
const COURSES = [
  { name: 'Nghệ thuật đệm hát',  done: 5,  total: 24, active: true  },
  { name: 'Hiểu biết âm nhạc',   done: 5,  total: 18, active: true  },
  { name: 'Nghệ thuật tỉa nốt',  done: 0,  total: 20, active: false },
  { name: 'Con đường nghệ sĩ',   done: 0,  total: 15, active: false },
]
const TASKS = [
...
function Bar({ pct, color = D.accent }: { pct: number; color?: string }) {
  return (
    <div style={{ height: 3, background: D.border, borderRadius: 99 }}>
      <div style={{ height: '100%', width: `${Math.max(pct > 0 ? pct : 0, 0)}%`, minWidth: pct > 0 ? 6 : 0, background: color, borderRadius: 99, transition: 'width .6s ease' }} />
    </div>
  )
}
    ...
    <div style={{ padding: '18px 20px 14px', borderBottom: `1px solid ${D.border}` }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 13, color: D.text1, letterSpacing: '.02em' }}>{title}</div>
            <div style={{ fontSize: 11, color: D.text3, marginTop: 1 }}>{sub}</div>
          </div>
        </div>
        {(badge || action) && (
          <div style={{ fontSize: 11, color: D.accent, fontWeight: 500, cursor: 'pointer', marginTop: 2 }}>
            {badge ?? action} →
          </div>
        )}
      </div>
...
export default function StudentPortalV2({ student, onLogout }: Props) {
  ...
  const card: React.CSSProperties = {
    background: D.surface, borderRadius: 12, boxShadow: D.shadow, overflow: 'hidden',
  }

  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: D.bg, fontFamily: '"Inter", "SF Pro Display", system-ui, sans-serif', color: D.text1, fontSize: 14 }}>
      ...
      <aside style={{ width: collapsed ? 52 : 200, flexShrink: 0, background: D.surface, borderRight: `1px solid ${D.border}`, display: 'flex', flexDirection: 'column', transition: 'width .2s ease', overflow: 'hidden' }}>
        ...
        <nav style={{ flex: 1, padding: '4px 8px' }}>
          {NAV.map(item => (
            <div key={item.label}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: collapsed ? '10px' : '9px 10px', borderRadius: 8, cursor: 'pointer', background: item.active ? D.accentLight : 'transparent', color: item.active ? D.accent : D.text2, fontWeight: item.active ? 600 : 400, whiteSpace: 'nowrap', overflow: 'hidden', marginBottom: 1, justifyContent: collapsed ? 'center' : 'flex-start', transition: 'background .1s' }}
              ...
              <span style={{ fontSize: 15, flexShrink: 0, opacity: item.active ? 1 : .7 }}>{item.icon}</span>
        ...
        <div style={{ borderTop: `1px solid ${D.border}`, padding: '8px' }}>
          {NAV_BOTTOM.map(item => (
            <div key={item.label}
              style={{ display: 'flex', alignItems: 'center', gap: 10, padding: collapsed ? '10px' : '9px 10px', borderRadius: 8, cursor: 'pointer', color: D.text3, whiteSpace: 'nowrap', overflow: 'hidden', justifyContent: collapsed ? 'center' : 'flex-start' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <header style={{ background: D.surface, borderBottom: `1px solid ${D.border}`, padding: '0 24px', height: 54, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: D.text3, fontSize: 18, padding: '4px 6px', borderRadius: 6 }}>🔔</button>

            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 10px 4px 4px', borderRadius: 8, border: `1px solid ${D.border}`, cursor: 'pointer', background: D.surface }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: D.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: D.accent, fontWeight: 700 }}>
                {name.charAt(0)}
              </div>
              <span style={{ fontWeight: 600, fontSize: 13, color: D.text1 }}>{name}</span>
              <span style={{ color: D.text3, fontSize: 11 }}>▾</span>
            </div>

            <button onClick={onLogout}
              style={{ background: 'none', border: `1px solid ${D.border}`, borderRadius: 7, padding: '5px 12px', fontSize: 12, color: D.text2, cursor: 'pointer', fontFamily: 'inherit' }}>
              Đăng xuất
            </button>
          </div>
        </header>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px 40px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ ...card, display: 'grid', gridTemplateColumns: '1fr 1fr 2fr 140px' }}>
            <div style={{ padding: '18px 20px', borderRight: `1px solid ${D.border}`, display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, background: D.accentLight, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: D.accent, fontWeight: 700, flexShrink: 0 }}>
                {name.charAt(0)}
              </div>
              <div>
                <div style={{ fontSize: 10, color: D.text3, marginBottom: 2 }}>Xin chào</div>
                <div style={{ fontWeight: 800, fontSize: 16, color: D.text1, lineHeight: 1.1 }}>{name}</div>
                <div style={{ fontSize: 11, color: D.text3, marginTop: 3 }}>Học viên · 2 khóa</div>
              </div>
            </div>
            ...
            <div style={{ padding: '18px 20px', borderRight: `1px solid ${D.border}` }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 13, color: D.accent, marginBottom: 3 }}>NGHỆ THUẬT ĐỆM HÁT</div>
            ...
            <div style={{ padding: '18px 24px', borderRight: `1px solid ${D.border}`, display: 'flex', alignItems: 'center' }}>
              <div style={{ fontSize: 24, color: D.border, lineHeight: 1, marginRight: 8, marginTop: -4 }}>"</div>
              <div style={{ fontSize: 13, color: D.text2, lineHeight: 1.65, fontStyle: 'italic' }}>
                Âm nhạc không chỉ để học — mà để sống cùng mỗi ngày.
              </div>
            </div>

            <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
              <div style={{ fontSize: 24, fontWeight: 800, color: D.text1, lineHeight: 1 }}>0</div>
          ...
          <div style={{ textAlign: 'center', padding: '4px 0 2px' }}>
            <div style={{ fontSize: 18, fontWeight: 800, color: D.text1, letterSpacing: '.05em' }}>HỌC · TẬP · SỐNG CÙNG ÂM NHẠC</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
            ...
            {/* CỘT 1 */}
            <div style={card}>
              <ColHeader icon="🎓" title="HỌC KIẾN THỨC" sub="Hiểu · Biết · Nắm vững" badge="2 khóa đang học" />
              ...
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${D.border}` }}>
                ...
                {TASKS.map((t, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderTop: i > 0 ? `1px solid ${D.borderLight}` : 'none', cursor: 'pointer' }}
                    onMouseEnter={el => (el.currentTarget.style.background = D.bg)}
                    onMouseLeave={el => (el.currentTarget.style.background = 'transparent')}>
                    <div style={{ width: 8, height: 8, borderRadius: '50%', border: `2px solid ${D.border}`, flexShrink: 0 }} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, color: D.text1 }}>{t.name}</div>
              ...
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${D.border}` }}>
                ...
                {EVENTS.map((e, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderTop: i > 0 ? `1px solid ${D.borderLight}` : 'none' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 12, fontWeight: 600, color: D.text1 }}>{e.name}</div>
                      <div style={{ fontSize: 11, color: D.text3 }}>{e.date}</div>
                    </div>
                    <button style={{ background: 'none', border: `1px solid ${D.border}`, borderRadius: 6, padding: '4px 10px', fontSize: 11, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', color: D.text2, flexShrink: 0 }}>
                      Tham gia
                    </button>
                  </div>
                ))}
          ...
          <div style={{ ...card, display: 'flex', alignItems: 'center', padding: '14px 24px', gap: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: D.text1, textTransform: 'uppercase', letterSpacing: '.06em', marginRight: 28, flexShrink: 0 }}>Thành Quả</div>
