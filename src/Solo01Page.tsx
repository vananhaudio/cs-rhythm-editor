...
export default function Solo01Page() {
  ...
  const endTime = (() => {
    const [hh, mm] = SOLO01.startTime.split(':').map(Number)
    const t = hh * 60 + mm + SOLO01.durationMinutes
        ...
        <section className="solo01-hero">
          <div className="solo01-wrap">
            ...
            <div className="solo01-timechip">
              <span className="solo01-timechip-big">{SOLO01.startTime} – {endTime}</span>
              <span className="solo01-timechip-sub">tối <b>{SOLO01.weekdayLabel}</b> hằng tuần · <b>24 buổi · 3 chặng · 6 tháng</b></span>
            </div>
...
@media(max-width:480px){.solo01-topback{font-size:12px;padding:6px 11px;}.solo01-btn{padding:11px 18px;font-size:14px;}}
`
