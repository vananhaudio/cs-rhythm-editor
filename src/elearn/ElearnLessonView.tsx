...
export default function ElearnLessonView({ num, title, courseSlug = 'khoi-dau-dam-me', studentName, isDone, onComplete, onBack, onOpenTool }: Props) {
      ...
      {PSEQ.map(i => {
        ...
        return (
          <button key={i} onClick={disabled || !onTap ? undefined : () => onTap(i)}
            ...
            <div style={{ width: 22, flexShrink: 0, textAlign: 'center', fontFamily: 'JetBrains Mono', fontSize: 12.5, fontWeight: 800, color: reveal ? c : '#C9C0AF' }}>{reveal ? s.n : '?'}</div>
            <div style={{ flex: 1, height: w, borderRadius: 99, background: c }} />
            <div style={{ width: 48, flexShrink: 0, textAlign: 'right', fontFamily: 'JetBrains Mono', fontSize: 11, fontWeight: 700, color: c }}>{reveal ? `${s.vn}·${s.note}` : (done ? '✓' : '')}</div>
          </button>
        )
      })}
    </div>
      ...
      <div>
        ...
        {answered ? (
          <div style={{ marginTop: 14 }}>
            <div style={{ padding: '12px 14px', borderRadius: 12, fontSize: 13.5, fontWeight: 600, lineHeight: 1.5, background: ok ? ACCENT.s : '#FBEDE9', color: ok ? ACCENT.d : '#A03B1C' }}>
              {ok ? 'Chính xác! Dây 1 là dây mỏng nhất, luôn nằm trên cùng.' : 'Chưa đúng — dây 1 là dây mỏng nhất, nằm trên cùng. Thử lại nhé.'}
            </div>
            <button onClick={() => setPick(null)} style={{ marginTop: 10, padding: '9px 16px', border: 'none', borderRadius: 10, background: '#1C1A17', color: '#fff', fontFamily: 'inherit', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>Thử lại</button>
          </div>
        ) : (
          <div style={{ marginTop: 12, fontSize: 12.5, color: '#8A8478', textAlign: 'center' }}>Chạm dây bạn nghĩ là dây số 1 (mỏng nhất, trên cùng)</div>
        )}
      </div>
    )
  }
  ...
  const ToolLaunch = () => (
    <button onClick={() => onOpenTool('tuner')}
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#F4E9D8' }}>Mở Tuner</div>
        <div style={{ fontSize: 12, color: '#9A9082', marginTop: 2, lineHeight: 1.35 }}>Đồ nghề bạn giữ mãi — lên dây mỗi lần chơi</div>
      </div>
  ...
  const Listen8 = () => {
    const seg = (id: 'play' | 'ear', label: string) => (
      <button key={id} onClick={() => { setL8mode(id); setPlayWrong(null) }}
        style={{ flex: 1, padding: 9, border: 'none', borderRadius: 10, cursor: 'pointer', fontFamily: 'inherit', fontSize: 12.5, fontWeight: 700, background: l8mode === id ? '#fff' : 'transparent', color: l8mode === id ? '#1C1A17' : '#8A8478', boxShadow: l8mode === id ? '0 1px 3px rgba(0,0,0,.1)' : 'none' }}>{label}</button>
    )
    const tabs = (
      ...
      return (
        <div>{tabs}
          <div style={{ display: 'flex', alignItems: 'center', gap: 13, padding: 15, borderRadius: 14, background: '#1C1A17', marginBottom: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.05em', color: '#9A9082' }}>{`HÃY GẢY (bằng ${playStyle === 'pick' ? 'pick' : 'ngón tay'})`}</div>
              <div style={{ fontSize: 17, fontWeight: 800, color: '#F4E9D8', marginTop: 2 }}>{`Dây buông số ${tg.n} · ${tg.vn} (${tg.note})`}</div>
            </div>
            <div style={{ fontFamily: 'JetBrains Mono', fontSize: 13, fontWeight: 700, color: '#9A9082' }}>{`${playStep}/6`}</div>
          </div>
          <HStrings onTap={(i) => l8strum(i)} reveal target={target} doneSet={doneSet} wrong={playWrong} />
          {playWrong != null && (
            <div style={{ marginTop: 14, padding: '12px 14px', borderRadius: 12, background: '#FBEDE9', color: '#A03B1C', fontSize: 13, fontWeight: 600, lineHeight: 1.5 }}>
              {`Bạn gảy dây ${STR[playWrong].n} (${STR[playWrong].note}). Hãy gảy dây ${tg.n} — ${where}.`}
            </div>
          )}
          <div style={{ marginTop: 10, fontSize: 11.5, color: '#A8A294', textAlign: 'center' }}>Chạm dây để thực hành — mic sẽ tự chấm khi nhúng.</div>
        </div>
      )
    }
    ...
    return (
      <div>{tabs}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: '#8A8478' }}>{`Câu ${exRound}/5`}</div>
          <div style={{ fontFamily: 'JetBrains Mono', fontSize: 12.5, fontWeight: 700, color: ACCENT.a }}>{`Đúng ${exScore}`}</div>
        </div>
        <button onClick={exPlay} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: 18, border: 'none', borderRadius: 15, background: '#1C1A17', color: '#F4E9D8', fontFamily: 'inherit', fontSize: 16, fontWeight: 700, cursor: 'pointer', marginBottom: 14 }}>🔊 Nghe âm</button>
        <div style={{ fontSize: 13.5, fontWeight: 700, textAlign: 'center', marginBottom: 12, color: '#3A352C' }}>Âm vừa nghe là dây buông nào?</div>
          ...
          {STR.map((s, i) => {
            ...
            return (
              <button key={i} onClick={() => exPick(i)}
                ...
                <div style={{ fontSize: 12.5, fontWeight: 700, color: '#3A352C' }}>{`Dây ${s.n} · ${s.vn}`}</div>
              </button>
            )
          })}
        </div>
        {answered && (
          <div style={{ marginTop: 14 }}>
            <div style={{ padding: '12px 14px', borderRadius: 12, fontSize: 13, fontWeight: 600, lineHeight: 1.5, background: ok ? ACCENT.s : '#FBEDE9', color: ok ? ACCENT.d : '#A03B1C' }}>
              {ok ? `Chính xác — đó là dây ${tgE.n} (${tgE.vn} · ${tgE.note}).` : `Là dây ${tgE.n} (${tgE.vn} · ${tgE.note}). Bấm 🔊 nghe lại để ghi nhớ.`}
            </div>
            <button onClick={exNext} style={{ marginTop: 10, width: '100%', padding: 13, border: 'none', borderRadius: 12, background: ACCENT.a, color: '#fff', fontFamily: 'inherit', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>{exRound >= 5 ? 'Xem kết quả' : 'Câu tiếp →'}</button>
          </div>
        )}
      </div>
    )
    ...
    const chip = (id: PlayStyle, emoji: string, label: string, sub: string) => (
      <button key={id} onClick={() => setPlayStyle(id)}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: '#1C1A17' }}>{label}</div>
          <div style={{ fontSize: 10.5, color: '#8A8478', marginTop: 1 }}>{sub}</div>
        </div>
        {playStyle === id && <div style={{ color: ACCENT.a, fontWeight: 800, fontSize: 14 }}>✓</div>}
      </button>
    )
    return (
  ...
  const NoteChart = () => {
    ...
    return (
      <div style={{ marginTop: 16, background: '#fff', border: '1px solid #EAE4D8', borderRadius: 16, padding: '14px 14px 12px' }}>
        ...
        <div style={{ fontSize: 11.5, color: '#8A8478', lineHeight: 1.45, marginBottom: 12 }}>Guitar dùng 7 chữ cái quốc tế. Đây là bảng "phiên dịch" sang Đô-Rê-Mi quen thuộc:</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 6 }}>
          {N.map(([letter, vn]) => (
            <div key={letter} style={{ textAlign: 'center', padding: '8px 2px', borderRadius: 10, background: used[letter] ? ACCENT.s : '#F6F2EA', border: used[letter] ? `1.5px solid ${ACCENT.a}` : '1px solid #EAE4D8' }}>
              <div style={{ fontFamily: 'JetBrains Mono', fontSize: 16, fontWeight: 800, color: used[letter] ? ACCENT.a : '#1C1A17' }}>{letter}</div>
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#F6F2EA', fontFamily: '"DM Sans", system-ui, sans-serif', color: '#1C1A17' }}>
      ...
      <div style={{ flexShrink: 0, background: 'rgba(246,242,234,.92)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', padding: 'calc(env(safe-area-inset-top,0px) + 12px) 16px 12px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid #E8E2D6' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: ACCENT.a, letterSpacing: '.03em' }}>{L.crumb}</div>
          <div style={{ fontSize: 14.5, fontWeight: 800, letterSpacing: '-.01em', lineHeight: 1.2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{L.title}</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px 20px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '13px 15px', background: ACCENT.s, borderRadius: 14 }}>
          ...
          <div style={{ fontSize: 13.5, color: '#3E463F', lineHeight: 1.5, fontWeight: 600 }}>{L.goal}</div>
        </div>
            ...
            {L.steps.map((t, i) => (
              <div key={i} style={{ display: 'flex', gap: 13, paddingBottom: 16 }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                  <div style={{ width: 26, height: 26, borderRadius: 99, background: '#1C1A17', color: '#F4E9D8', fontSize: 12.5, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{i + 1}</div>
                  <div style={{ width: 2, flex: 1, background: i < L.steps.length - 1 ? '#E2DBCD' : 'transparent', marginTop: 3, minHeight: 8 }} />
                </div>
                <div style={{ fontSize: 14, color: '#3A352C', lineHeight: 1.55, paddingTop: 3 }}>{t}</div>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={{ marginTop: 24 }}>
          <div style={{ fontSize: 11.5, fontWeight: 800, color: '#8A8478', letterSpacing: '.05em' }}>THAO TÁC NGAY</div>
          <div style={{ fontSize: 14.5, fontWeight: 700, margin: '5px 0 14px', lineHeight: 1.35 }}>{L.prompt}</div>
        ...
        <div style={{ margin: '18px 0 26px', display: 'flex', alignItems: 'center', gap: 11, padding: '13px 15px', background: '#F1ECE2', borderRadius: 14 }}>
          <div style={{ width: 32, height: 32, flexShrink: 0, borderRadius: 99, background: 'linear-gradient(135deg,#2C2823,#5A5043)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#F4E9D8', fontWeight: 700, fontSize: 11.5 }}>VA</div>
          <div style={{ fontSize: 12.5, color: '#5A5448', lineHeight: 1.45 }}>Thấy khó ở đâu? <b>Nhắn thầy Văn Anh</b> — vấp là chuyện bình thường.</div>
        </div>
      </div>
      ...
      <div style={{ flexShrink: 0, padding: 'calc(env(safe-area-inset-bottom,0px) + 14px) 20px calc(env(safe-area-inset-bottom,0px) + 14px)', background: 'linear-gradient(0deg,#F6F2EA 72%,rgba(246,242,234,0))' }}>
        <button onClick={onComplete} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, padding: 15, border: 'none', borderRadius: 15, background: ACCENT.a, color: '#fff', fontFamily: 'inherit', fontSize: 15, fontWeight: 700, cursor: 'pointer', boxShadow: `0 12px 26px -12px ${ACCENT.a}` }}>
          {ctaLabel}
