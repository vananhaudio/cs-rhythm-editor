...
// durSec = thời lượng nốt cần NGÂN (theo trường độ + tốc độ). Nốt trắng/tròn phải kêu dài,
// không tự tắt sớm. Mặc định 1.7s (giữ hành vi cũ khi gọi không truyền — vd tiếng xác nhận).
export function playTone(freq: number, durSec = 1.7) {
  const ac = ctx()
  if (!ac) return
  try {
    cutActive(ac)
    const t = ac.currentTime
    const ring = Math.max(0.4, Math.min(durSec, 6))          // ngân đúng trường độ (0.4–6s)
    const lp = ac.createBiquadFilter(); lp.type = 'lowpass'
    lp.frequency.setValueAtTime(3600, t)
    lp.frequency.exponentialRampToValueAtTime(700, t + Math.min(1.3, ring))
    const g = ac.createGain()
    g.gain.setValueAtTime(0.0001, t)
    g.gain.exponentialRampToValueAtTime(0.5, t + 0.012)                       // gảy — attack nhanh
    g.gain.exponentialRampToValueAtTime(0.16, t + Math.min(0.45, ring * 0.4)) // vào thân nốt
    g.gain.exponentialRampToValueAtTime(0.0008, t + ring)                     // ngân tắt dần tới hết trường độ
    ...
    ;([[1, 'sawtooth', 0.6], [2, 'sine', 0.22], [3, 'sine', 0.1]] as const).forEach(([h, ty, amp]) => {
      ...
      o.connect(og); og.connect(lp); o.start(t); o.stop(t + ring + 0.1)
      oscs.push(o)
    })
