...
const SLIDE_TYPES = [
  ...
  { id: 'support',     icon: '🧭', label: 'Gỡ rối & Đào sâu' },
...
function Label({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 12, fontWeight: 600, color: C.text2, marginBottom: 5, textTransform: 'uppercase', letterSpacing: .4 }}>{children}</div>
}
function Inp({ value, onChange, placeholder, style }: { value: string; onChange: (v: string) => void; placeholder?: string; style?: React.CSSProperties }) {
  return (
    <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      style={{ width: '100%', boxSizing: 'border-box', padding: '7px 10px', border: `1px solid ${C.border}`, borderRadius: 7, fontSize: 14, color: C.text1, fontFamily: 'inherit', outline: 'none', background: C.surface, ...style }} />
  )
}
function Sel({ value, onChange, children, style }: { value: string; onChange: (v: string) => void; children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)}
      style={{ width: '100%', padding: '7px 10px', border: `1px solid ${C.border}`, borderRadius: 7, fontSize: 14, color: C.text1, background: C.surface, fontFamily: 'inherit', outline: 'none', ...style }}>
      {children}
    </select>
  )
}
function Btn({ onClick, children, variant = 'secondary', style }: { onClick: () => void; children: React.ReactNode; variant?: 'primary' | 'secondary' | 'danger'; style?: React.CSSProperties }) {
  ...
  return (
    <button onClick={onClick}
      style={{ padding: '7px 14px', borderRadius: 8, border: `1px solid ${variant === 'secondary' ? C.border : bg}`, background: bg, color, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', ...style }}>
      {children}
    </button>
  )
}
      ...
      {STRINGS.map(s => {
        ...
        return (
          <button key={s.num} onClick={() => onTap(s.num)}
            ...
            <span style={{ width: 16, fontFamily: 'ui-monospace, monospace', fontSize: 13, fontWeight: 800, color: colorOfNum(s.num) }}>{s.num}</span>
            <span style={{ flex: 1, height: 3 + (s.num - 1) * 1.4, borderRadius: 99, background: colorOfNum(s.num) }} />
            <span style={{ width: 52, textAlign: 'right', fontFamily: 'ui-monospace, monospace', fontSize: 12, fontWeight: 700, color: C.text2 }}>{s.vn}·{s.note}</span>
            {on && <span style={{ width: 18, fontSize: 12, fontWeight: 800, color: C.success }}>{idx >= 0 && selected.length > 1 ? idx + 1 : '✓'}</span>}
          </button>
        )
      })}
    </div>
...
export default function FlowInlineEditor({ lessonId }: Props) {
  ...
  if (loading) return <div style={{ padding: 16, color: C.text3, fontSize: 14 }}>Đang tải...</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px', gap: 10 }}>
        ...
        <div>
          ...
          <input type="number" value={rewardXp} onChange={e => setRewardXp(Number(e.target.value))} min={0}
            style={{ width: '100%', boxSizing: 'border-box', padding: '7px 10px', border: `1px solid ${C.border}`, borderRadius: 7, fontSize: 14, color: C.text1, fontFamily: 'inherit', outline: 'none' }} />
        </div>
      </div>
      ...
      <div>
        ...
        {slides.length === 0 && (
          <div style={{ textAlign: 'center', padding: '24px 0', color: C.text3, fontSize: 14, border: `1.5px dashed ${C.border}`, borderRadius: 10 }}>
            ...
            return (
              <div key={s.id}>
                ...
                <div onClick={() => setEditSlide(isEditing ? null : s)}
                  ...
                  <span style={{ color: C.text3, fontSize: 12, width: 18, textAlign: 'center', flexShrink: 0 }}>{i + 1}</span>
                  <span style={{ background: lm.bg, color: lm.color, borderRadius: 99, padding: '2px 8px', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{lm.label}</span>
                  <span style={{ fontSize: 15, flexShrink: 0 }}>{TYPE_ICON[s.type] ?? '📄'}</span>
                  <span style={{ fontSize: 13, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: isEditing ? C.accent : C.text1, fontWeight: isEditing ? 600 : 400 }}>
                  ...
                  <button onClick={e => { e.stopPropagation(); moveSlide(s.id, -1) }}
                    disabled={i === 0}
                    style={{ background: 'none', border: 'none', cursor: i === 0 ? 'default' : 'pointer', color: C.text3, fontSize: 14, padding: '0 2px', opacity: i === 0 ? 0.3 : 1 }}>↑</button>
                  <button onClick={e => { e.stopPropagation(); moveSlide(s.id, 1) }}
                    disabled={i === slides.length - 1}
                    style={{ background: 'none', border: 'none', cursor: i === slides.length - 1 ? 'default' : 'pointer', color: C.text3, fontSize: 14, padding: '0 2px', opacity: i === slides.length - 1 ? 0.3 : 1 }}>↓</button>
                  <button onClick={e => { e.stopPropagation(); duplicateSlide(s.id) }}
                    title="Nhân đôi slide"
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.text3, fontSize: 13, padding: '0 3px', lineHeight: 1 }}
                    onMouseEnter={e => (e.currentTarget.style.color = C.accent)}
                    onMouseLeave={e => (e.currentTarget.style.color = C.text3)}>⊕</button>
                  <button onClick={e => { e.stopPropagation(); removeSlide(s.id) }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.text3, fontSize: 16, padding: '0 3px', lineHeight: 1 }}
                ...
                {isEditing && editSlide && (
                  <div style={{ border: `1px solid ${C.accent}`, borderTop: 'none', borderRadius: '0 0 10px 10px', padding: 16, background: '#FAFBFF', display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 4 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                      <div>
                        ...
                        <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ background: lm!.bg, color: lm!.color, borderRadius: 99, padding: '3px 12px', fontSize: 12, fontWeight: 700 }}>{lm!.label}</span>
                        </div>
                      </div>
                      <div>
                    ...
                    {editSlide.type === 'note_chart' && (
                      <div style={{ fontSize: 13, color: C.text3, background: '#FAFBFF', border: `1px solid ${C.border}`, borderRadius: 8, padding: '10px 12px' }}>
                    ...
                    {editSlide.type === 'guitar_strum' && (() => {
                      ...
                      return (
                        <div>
                          ...
                          <div style={{ fontSize: 13, color: seq.length ? C.text1 : C.text3, marginBottom: 6 }}>
                    ...
                    {editSlide.type === 'note_show' && (() => {
                      ...
                      return (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                          ...
                          <label style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 13.5, color: C.text2, cursor: 'pointer' }}>
                    ...
                    {editSlide.type === 'note_practice' && (() => {
                      ...
                      return (
                        <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 10 }}>
                          ...
                          <div style={{ gridColumn: '1 / -1', fontSize: 12.5, color: C.text3, lineHeight: 1.5 }}>
                    ...
                    {editSlide.type === 'guitar_ear' && (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                        ...
                        <div style={{ alignSelf: 'end', fontSize: 12.5, color: C.text3, paddingBottom: 7 }}>Nghe âm → đoán dây (cả 6 dây)</div>
                      </div>
                    )}
                    ...
                    {editSlide.type === 'support' && (
                      <div>
                        ...
                        <div style={{ fontSize: 12.5, color: C.text3, marginTop: 6, lineHeight: 1.5 }}>
                    ...
                    {editSlide.type === 'checklist' && (
                      <div>
                        ...
                        <textarea
                          ...
                          style={{ width: '100%', boxSizing: 'border-box', padding: '8px 10px', border: `1px solid ${C.border}`, borderRadius: 7, fontSize: 14, fontFamily: 'inherit', outline: 'none', resize: 'vertical', color: C.text1 }} />
                        <label style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 8, fontSize: 13.5, color: C.text2, cursor: 'pointer' }}>
      ...
      <div style={{ paddingTop: 4, borderTop: `1px solid ${C.border}` }}>
        ...
        {flowId && <div style={{ textAlign: 'center', fontSize: 12, color: C.success, marginTop: 6 }}>✓ Flow đã tồn tại · {slides.length} slides</div>}
      </div>
        ...
        <div onClick={() => setShowPreview(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #EEE', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              <span style={{ fontWeight: 700, fontSize: 15, color: C.text1 }}>📱 Xem thử — {flowTitle || 'Flow chưa lưu'}</span>
              <button onClick={() => setShowPreview(false)}
                style={{ background: '#F4F4F5', border: 'none', borderRadius: 8, padding: '5px 12px', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, color: C.text2 }}>
                Đóng ✕
              </button>
            </div>
      ...
      {showImport && (
        <div onClick={() => setShowImport(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 4 }}>📋 Import slides từ JSON</div>
            ...
            <button onClick={() => jsonFileRef.current?.click()}
              style={{ width: '100%', padding: '9px 0', marginBottom: 10, border: `1.5px dashed ${C.border}`, borderRadius: 8, background: '#FAFBFF', color: C.accent, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
            ...
            <textarea value={importText} onChange={e => { setImportText(e.target.value); setImportError('') }} rows={9}
              ...
              style={{ width: '100%', boxSizing: 'border-box', padding: '10px 12px', border: `1px solid ${importError ? C.danger : C.border}`, borderRadius: 8, fontSize: 13, fontFamily: 'ui-monospace, monospace', outline: 'none', resize: 'vertical', color: C.text1 }} />
            {importText && !importError && (
              <div style={{ fontSize: 12, color: C.success, marginTop: 4 }}>✓ Đã có nội dung — bấm "Nhập slides" để import</div>
            )}
            {importError && <div style={{ color: C.danger, fontSize: 13, marginTop: 6 }}>{importError}</div>}
