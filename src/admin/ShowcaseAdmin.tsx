...
.sa-close { background: none; border: none; color: #2D6A4F; font-size: 14px; font-weight: 500; cursor: pointer; padding: 0; margin-bottom: 8px; }
.sa-close:hover { text-decoration: underline; }
.sa-title { font-size: 28px; font-weight: 800; color: #18181B; margin: 0; }

.sa-nav-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
.sa-nav-card {
  display: flex; flex-direction: column; align-items: center; gap: 8px;
  padding: 32px 20px; background: #fff; border: 1px solid #E4E4E7; border-radius: 14px;
  cursor: pointer; font-family: inherit; text-align: center;
  transition: border-color .15s, box-shadow .15s;
}
.sa-nav-card:hover { border-color: #2D6A4F; box-shadow: 0 4px 20px rgba(45,106,79,0.1); }
.sa-nav-icon { font-size: 36px; }
.sa-nav-label { font-size: 16px; font-weight: 700; color: #18181B; }
.sa-nav-desc { font-size: 12px; color: #71717A; line-height: 1.5; }

.sa-preview { margin-top: 32px; text-align: center; }
.sa-preview-hint { font-size: 13px; color: #71717A; }
.sa-preview-hint a { color: #2D6A4F; font-weight: 500; }
