...
export function parseHopAmVietWithBeatsPerChord(
  ...
  for (const seg of segments) {
    if (seg.chordName === null) {
      ...
      // Pickup lyrics phân bổ đều trong bar đầu
      seg.lyrics.forEach((w, i) => {
        const pickupBeat = i * timeSig / Math.max(seg.lyrics.length, 1);
        tokens.push({ kind: 'lyric', text: w, beatOffset: pickupBeat });
      });
    } else {
      ...
      // Phân bổ lời đều trong slot — mỗi từ cách nhau slotBeats/count
      seg.lyrics.forEach((w, i) => {
        const lyricBeat = cb + (i * slotBeats / Math.max(seg.lyrics.length, 1));
