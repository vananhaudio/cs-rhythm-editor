...
const LEVELS = [
  { lvl:1, name:'Level 1: Một hợp âm · Quạt đều · Nghe chủ động', lessons:[
    { slug:'dh11', n:1, title:'Bài 1.1 — Am, một tiếng sạch', xp:20, slides:[
      ...
      t('NHAN','④ Nghe', box('🎧 Trên App','Nghe hai mẫu — một tiếng Am sạch và một tiếng Am bị tịt — tập phân biệt tới khi nghe ra ngay. Có <b>video hướng dẫn mẫu</b> cách bấm Am sạch.','#3E7C74','#EAF3F1')),
  ...
  { lvl:2, name:'Level 2: Đổi hai hợp âm · Nhịp không vỡ', lessons:[
    { slug:'dh21', n:1, title:'Bài 2.1 — Hai thế & tư duy ngón', xp:20, slides:[
      ...
      t('NHAN','④ Nghe', box('🎧 Trên App','Nghe mẫu C sạch; vừa bấm vừa nghe: khi ghì quá chặt tiếng có gì khác so với bấm vừa đủ? Có <b>video hướng dẫn mẫu</b> ngón trụ Am↔C.','#3E7C74','#EAF3F1')),
      embedChord('C'),
    ...
    { slug:'dh22', n:2, title:'Bài 2.2 — Nhấc & chuẩn bị: vào đúng phách', xp:20, slides:[
      ...
      metro('④⑤ Metronome ~60. Tập |Am · · · | C · · · | thật chậm, bắt đầu chuyển ngay sau phách 4; nghe C có vang đúng tiếng click phách 1 không. Có <b>video hướng dẫn mẫu</b> ngón bay.'),
    ...
    { slug:'dh27', n:7, title:'Bài 2.7 — Bắt đầu từ dây bass (chặn ngón cái)', xp:30, slides:[
      ...
      t('NHAN','④ Nghe', box('🎧 Trên App','Nghe C quạt cả 6 dây (đục) và C chặn dây 6 (gọn); nghe Am → E bass “rơi xuống” dây 6. Có <b>video hướng dẫn mẫu</b> chặn ngón cái (thumb-mute).','#3E7C74','#EAF3F1')),
  ...
  { lvl:3, name:'Level 3: Vòng hoà âm · Biết mình đang ở đâu', reuseModule:'319654a9-f926-4b68-8adb-d5a1f566718d', lessons:[
    { slug:'dh30', n:0, title:'Bài 3.0 — Ôn dây 6 & hợp âm mới E7', xp:20, slides:[
      ...
      t('NHAN','④ Nghe', box('🎧 Trên App','Nghe E7 và so với E; nghe bass dây 6 “xuống sâu”. Có <b>video hướng dẫn mẫu</b> phạm vi quạt dây 6.','#3E7C74','#EAF3F1')),
  ...
  { lvl:5, name:'Level 5: Tự chuẩn bị & đệm một bài hát mới', lessons:[
    { slug:'dh51', n:1, title:'Bài 5.1 — Tìm tông dễ chơi', xp:20, slides:[
      ...
      hieu(P('Đưa về Am/C không làm đổi bài — chỉ đổi “chỗ đứng” để tay dễ chơi. Cao độ để hát sẽ chỉnh bằng capo ở bài sau.') + box('Tôi muốn bạn nhớ','Đích của Đệm hát 1 không phải thuộc tất cả hợp âm, mà là biết cách xử lý khi gặp một hợp âm chưa biết. Dịch tông + capo đưa bài về vốn đã học; gặp hợp âm lạ thì tra cứu (Bài 5.3).','#9C4668','#F3E9F0')),
    ...
    { slug:'dh53', n:3, title:'Bài 5.3 — Biên soạn bảng hợp âm', xp:20, slides:[
      ...
      hoc(box('Nếu gặp một hợp âm chưa biết','Đừng bỏ bài. Làm theo: 1) Xác định tên hợp âm. 2) Tra cứu thế bấm trên Internet. 3) Chọn nguồn đáng tin cậy. 4) Đọc sơ đồ. 5) Bấm thử, gảy rời kiểm tra từng dây. 6) Luyện chuyển với một hợp âm quen. 7) Đưa vào bài, đặt đúng phách. — Người đệm hát không cần biết sẵn mọi hợp âm, nhưng phải biết làm gì khi gặp một hợp âm mới.','#9C4668','#F3E9F0')),
      dat('Có một bảng hợp âm sạch, dễ đọc; và biết cách tra cứu khi gặp một hợp âm chưa biết.'),
    ]},
    { slug:'dh54', n:4, title:'Bài 5.4 — Tìm phách rồi điền vào lời', xp:20, slides:[
      kham(P('Mở một bài hát bạn thích, vừa nghe vừa vỗ tay. Vỗ vào đâu thấy “chắc tay” nhất? Đó là phách mạnh; những tiếng nhẹ xen giữa là phách nhẹ.')),
      hoc(box('Bước 1 — phách MẠNH bằng mắt','Nhìn bản nhạc: số chỉ nhịp (2/4, 3/4, 4/4…) và các vạch nhịp. Ngay sau mỗi vạch nhịp là phách 1 — phách mạnh của ô. Chưa cần đọc hết tiết tấu; chỉ cần thấy cấu trúc ô nhịp để biết nhịp bắt đầu ở đâu.','#3E7C74','#EAF3F1')),
      hoc(box('Bước 2 — phách NHẸ bằng tai','Phách nhẹ khó “đọc” hết bằng ký hiệu ở trình độ này. Hãy tự hát → vỗ tay → nghe → cảm nhận. Để ý ca sĩ bắt đầu tiếng ở đâu, kéo dài ở đâu, câu hát “rơi” vào đâu. <b>Phách mạnh tìm bằng mắt; phách nhẹ phải học cách tìm bằng tai.</b>','#3E7C74','#EAF3F1')),
      metro('④⑤ Metronome: khi đã cảm được phách, điền số phách cho lời và đánh dấu chỗ đổi hợp âm — kể cả khi đổi giữa câu.'),
      dat('Nhìn bản nhạc tìm được phách mạnh; hát + vỗ tay + nghe để cảm phách nhẹ; rồi điền phách vào lời và xác định đúng chỗ đổi hợp âm (kể cả đổi giữa câu).'),
    ]},
    ...
    { slug:'dh56', n:6, title:'Bài 5.6 — Vào hát từ hợp âm', xp:20, slides:[
      ...
      listen({ q:'Để cất giọng vào ĐÚNG cao độ, nên làm gì trước?', plays:[{label:'Nghe hợp âm đầu (Am)', chords:[fr('Am')]}], options:['Đoán đại rồi hát','Nghe hợp âm, lấy một nốt trong đó làm “điểm tựa” rồi vào','Hát thật to cho chắc'], answer:1, explain:'Hợp âm đang vang cho bạn một “nốt mồi”. Ngân nhẹ nốt đó (như tiếng “ừm” ở Level 1) rồi vào câu hát — không bị chênh. Có video hướng dẫn mẫu.' }),
    ...
    { slug:'dh58', n:8, title:'Bài 5.8 — Ghép đàn', xp:30, slides:[
      ...
      metro('⑤ Backing/metronome nền: hát + đệm cả bài; đổi hợp âm đúng phách đã điền, tay phải giữ đều. Có <b>video hướng dẫn mẫu</b> cách ghép đàn.'),
    ...
    { slug:'dh59', n:9, title:'Bài 5.9 — Một bài hát hoàn toàn mới (Capstone)', xp:50, slides:[
      ...
      hoc(box('Checklist quy trình','1) Chọn tông dễ (Am/C). 2) Chọn capo hợp giọng. 3) Chỉnh & tra cứu hợp âm (gặp hợp âm lạ thì tra thế bấm). 4) Xác định nhịp. 5) Tìm phách mạnh (nhìn sheet). 6) Tìm phách nhẹ (hát + vỗ tay + nghe). 7) Điền phách vào lời. 8) Xác định điểm đổi hợp âm. 9) Hát + vỗ tay + metronome (chưa đàn). 10) Vào câu hát đúng cao độ. 11) Nhận âm chủ / hợp âm chủ. 12) Ghép đàn → tự đệm hoàn chỉnh.')),
      hieu(P('Đây là đích của cả Đệm hát 1: bạn không cần thuộc bài trước, vẫn tự đệm được một bài mới.')),
      metro('⑤ Làm đủ 12 bước trên một bài mới → đệm hoàn chỉnh. Tự chấm từng bước; nghe lại bản đệm của mình.'),
      dat('Nhận một bài chưa từng học và tự: tông/capo → chỉnh & tra cứu hợp âm lạ → xác định nhịp → tìm phách mạnh (mắt) và phách nhẹ (tai) → điền phách & điểm đổi → hát+vỗ tay+metronome → vào câu hát đúng cao độ → nhận âm chủ → ghép đàn & đệm hoàn chỉnh.'),
    ]},
