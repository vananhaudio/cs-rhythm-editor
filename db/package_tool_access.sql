...
begin
 if not exists(select 1 from public.edu_tools t where t.route like '/%' and rtrim(split_part(t.route,'?',1),'/')=rtrim(p_path,'/')) then return true; end if;
 v_state:=public.my_learning_state();
 if v_state->>'mode'='teacher' then return true; end if;
 return exists(select 1 from public.edu_tools t
 where t.route like '/%' and rtrim(split_part(t.route,'?',1),'/')=rtrim(p_path,'/')
 and coalesce((v_state->'flags'->'tools'->>t.id)::boolean,false));
end $$;
