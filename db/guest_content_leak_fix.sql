-- Chặn khách chưa đăng nhập đọc nội dung khoá bán tiền
-- Trước: bài để tier='free' trong khoá is_free=false vẫn 'open' cho anon — DH1 lộ 44 content_url,
-- DH2 lộ 11, TN3 lộ 2, TN1 mở 62 bài dạng flow. Lỗi có sẵn từ trước (đã đo A/B xác nhận).
-- Sau: khách chỉ học được nội dung miễn phí CÓ CHỦ Ý (khoá is_free hoặc chương is_free do Admin bật).
-- Học viên đã đăng nhập KHÔNG đổi gì. Bản gốc lưu ở db/_backup_my_learning_state_truoc_2026-09-20.sql
...
declare
  ...
  v_is_guest boolean := (auth.uid() is null);   -- khách chưa đăng nhập
      ...
      'lessons', coalesce((
        select jsonb_agg(jsonb_build_object(
            ...
            'access', case
              ...
              -- KHÁCH chưa đăng nhập: chỉ được học nội dung miễn phí CÓ CHỦ Ý
              -- (khoá is_free hoặc chương is_free do Admin bật). Bài lẻ để tier='free'
              -- trong khoá bán tiền KHÔNG còn mở ra ngoài — trước đây lộ cả content_url.
              when v_is_guest
                   and not (coalesce(cr.is_free, false) or coalesce(m.is_free, false))
                then 'upgrade'
