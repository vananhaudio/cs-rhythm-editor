...
- **SERVER-DRIVEN LEARNING STATE (từ 09/2026):** quyền học/hành trình/khoá-mở-bài do RPC `my_learning_state()` quyết (xem `docs/SERVER_DRIVEN_ARCHITECTURE.md` + `db/learning_state_setup.sql`). **Client MUST NOT tự suy luận quyền từ enrollment/packages/policy** — `contentAccess.ts`/`hanhtrinh.ts` chỉ là fallback legacy, không mở rộng. Đổi luật quyền → sửa RPC + chạy `db/tests/learning_state_test.sql`. Hành trình/tiên quyết sửa ở Admin → Hành trình (`journey_curriculum`/`course_prereqs`), KHÔNG hardcode UUID trong bundle. Trước khi code bất kỳ yêu cầu nào phải hỏi: *đây là DATA/CONFIG/POLICY (→ server/admin) hay CAPABILITY mới (→ code)?*
...
- **KIẾN TRÚC (từ 1.2): BUNDLED WEB ASSETS.** `capacitor.config.json` **KHÔNG bật `server.url`** (chỉ `appId`/`appName`/`webDir: dist` + từ 1.3.0 có `server.iosScheme: "https"` — origin iOS là `https://localhost` để YouTube chấp nhận embed; ĐỪNG bỏ dòng này: bỏ là video chết và học viên bị logout thêm lần nữa do đổi origin). App production iOS **và** Android chạy bản web **đóng gói trong native**, KHÔNG tải web live. ⇒ **Deploy web KHÔNG tự cập nhật app đã cài.** Mọi thay đổi UI muốn tới người dùng store phải: `npm run build` → `npx cap copy ios/android` (bake `dist` vào native) → tăng version/build → build lại native → nộp lại store. ĐỪNG bật lại `server.url`.
