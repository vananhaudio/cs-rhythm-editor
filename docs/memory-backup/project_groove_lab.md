...
- VERSION: 1.0.0 đã duyệt công khai App Store (build 7). Đợt thêm tab Tập (Nền tập quạt) = **1.1.0** — ĐÃ DUYỆT & LIVE CÔNG KHAI cả App Store + CH Play (2026-06-30). Tinh chỉnh tiếp (Bolero hi-hat đơn-kép-kép, lưới 16) = **1.1.1** (mới TestFlight, để dành update sau). LƯU Ý: eas submit thỉnh thoảng lỗi "Something went wrong" transient → cứ submit lại là được. LƯU Ý: version 1.0.0 đã bị Apple "khoá train" sau khi duyệt → mỗi đợt tính năng mới PHẢI bump `version` trong app.json (1.1.0→1.2.0…), không chỉ buildNumber (EAS tự tăng). Quên bump version → lỗi 90186/90062 khi submit.
