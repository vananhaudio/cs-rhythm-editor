...
const BAR_HEIGHT = 100;    // chiều cao mỗi bar row
const BEAT_NUM_Y = 6;      // y số phách
const CHORD_Y = 26;        // y chord
const LYRIC_Y = 60;        // y lyric
...
function Chip({ ev, kind, label, x, y, snapMode, barStart, bd, timeSig, barDur, pxPerBeat,
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => {
      ...
      const snapped = Math.min(snapTime(rawTime, 60 / bd, timeSig, snapMode), barStart + barDur - 0.001);
    ...
    const onUp = (e: MouseEvent) => {
      ...
      const snapped2 = Math.min(snapTime(rawT, 60 / bd, timeSig, snapMode), barStart + barDur - 0.001);
...
function BarRow({ barIndex, song, snapMode, pxPerBeat, onTimeChange, onDelete, onEditLabel, onAdd }: {
  barIndex: number;
  song: RhythmSong;
  snapMode: SnapMode;
  pxPerBeat: number;
  ...
  const barChords = song.chords.filter(c => c.time >= barStart && c.time < barEnd);
  const barLyrics = song.lyrics.filter(l => l.time >= barStart && l.time < barEnd);
  ...
  return (
    ...
    >
      {/* ── Nhãn bar — góc trên trái, không chiếm không gian ── */}
          ...
          return (
            <div key={b} style={{
              ...
              <div style={{
                ...
                width: isStrong ? 2 : 1,
                height: '100%',
                background: isStrong ? '#ddd' : '#f0f0f0',
...
export default function App() {
  const [song, setSong] = useState<RhythmSong>(() => createEmptySong({ totalBars: 16 }));
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => { if (e.key === 'Escape') setAddTarget(null); };
  ...
  const handleExport = useCallback(() => { downloadJson(song); setIsDirty(false); }, [song]);

  const handleFileImport = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]; if (!file) return;
    const reader = new FileReader();
    reader.onload = ev => {
      try { setSong(importFromJson(ev.target?.result as string)); setIsDirty(false); }
  ...
  const handleClear = useCallback(() => {
    ...
    setSong(prev => ({ ...prev, lyrics: [], chords: [], updatedAt: new Date().toISOString() }));
    setIsDirty(true);
  }, []);
